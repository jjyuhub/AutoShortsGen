...
# ✅ Fix: Use a solid color background with a smooth wave animation
ffmpeg -f lavfi -i "color=c=black:s=1080x1920:d=15" -vf "format=yuv420p,mpdecimate,eq=contrast=1.2:brightness=0.05,deflicker" -t 15 -y "$OUTPUT_DIR/animated_bg.mp4"

# ✅ Fix: Add animated text overlay (moving up smoothly)
ffmpeg -i "$OUTPUT_DIR/animated_bg.mp4" -vf "
drawtext=text='$SCRIPT_TEXT':fontcolor=white:fontsize=48:x=(w-text_w)/2:y=h/2-50:enable='between(t,0,5)',
drawtext=text='Embrace automation now!':fontcolor=yellow:fontsize=42:x=(w-text_w)/2:y=h/2+50:enable='between(t,5,10)'" -c:a copy "$OUTPUT_DIR/animated_text.mp4"
...
# ✅ Fix: Animated subtitles for vertical format
